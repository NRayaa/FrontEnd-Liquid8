...
import { useEditProductApprovesMutation, useGetDetailProductApprovesByDocQuery, useGetDetailProductApprovesQuery, useGetDetailProductStaggingQuery } from '../../../../store/services/categoriesApi';
import { Alert } from '../../../../commons';
import IconArrowBackward from '../../../../components/Icon/IconArrowBackward';
import NewBarcodeData from '../NewBarcodeData';
import BarcodeData from '../BarcodeData';

const DetailProductStaggingApprovement = () => {
    const { id }: any = useParams();
    const navigate = useNavigate();
    const { data, isSuccess, isError } = useGetDetailProductStaggingQuery(id);
    // refetch
    ...
    useEffect(() => {
        if (results.isSuccess) {
            ...
            navigate('/inbound/check_product/approvment_product_stagging/detail', { state: { code_document: dataDetailProduct?.code_document } });
            // productNew.refetch();
        ...
        <>
            <ul className="flex space-x-2 rtl:space-x-reverse">
                ...
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                    <span>Detail Approvment Product Stagging</span>
                </li>
            </ul>{' '}
            <div className="mt-10 p-6 panel">
                <div className="flex items-center justify-between mb-6">
                    <h1 className="text-xl font-bold">Detail Product Stagging</h1>
                    <Link to="/inbound/check_product/approvment_product_stagging" state={{ code_document: dataDetailProduct?.code_document }}>
...
export default DetailProductStaggingApprovement;
